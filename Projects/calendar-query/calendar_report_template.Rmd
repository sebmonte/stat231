...
calendar_data <- "Projects/calendar-query/SMontesinosCalendarQuery.ics" %>% 
...
# Now that I have a wider format and 0s for activities I spent no time on, I can group by day and sum the activity time into categories I want to compare
activities_comparison2 <- activities_comparison %>%
  group_by(day) %>%
  summarise(Work_time = sum(thesis, oceanography, religion), Personal_Time = sum(personal), Extracurricular_time = sum(extracurriculars))
# 
