...
Group thoughts:
- Could have some offset system where people who stay safe on less safe roads get a bonus
- Could test the algorithm and look and its natural ratings before making a determination to see whether it would be biased
...
Group thoughts:
- Ideally you might want to publish the data anonymously so you do not disclose people's information but give people information about areas where speeding occurs. However, this could lead to labeling certain areas as more dangerous which could lead to more bias against poorer areas.
- Give the anonymized data to people conducting studies on ways to promote safe driving
...
Group thoughts:
- You would want to store it so the data that gets shared with other companies or for studies is anonymized so that you could never trace it back to a specific individual's identity
...
Group thoughts: 
- Maybe an algorithm could be more fair since it would be free from human bias, once we establish one
- Need admissions officers to agree on what is important and how important it is to program it into the algorithm so that it applies the same standards to everyone
- It would be good to make the algorithm blind to aspects like the person's ethnic or economic background, but at the same time it could be hard to ensure that it is
