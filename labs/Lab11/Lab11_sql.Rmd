...
Solution: No because not all of the commands below will get automatically translated into SQL commands, since they are not all from dplyr.
...
Solution: Groups by the session and looks for the average number of females per number of session, the average age per number of sessions, and the amount of patients who had that number of sessions.
...
Solution: WHERE will filter the result BEFORE you do any grouping and sort by certain variables, while HAVING while filter after the resulting dataframe has occurred ie. after grouping_by.
...
```{sql, connection = con, output.var = table_graph}
SELECT Frequency,
Absorbance,
Ear
FROM Measurements
...
```{r}
ggplot(data = table_graph, aes(x = Frequency,
                               y = Absorbance,
                               color = Ear)) +
  geom_line()
```
...
```{r}
result_table <- dbGetQuery(con, "SELECT Frequency,Absorbance,Ear
FROM Measurements
WHERE Identifier = 'Rosowski_2012' AND SubjectNumber = 3") %>%
  collect()


ggplot(data = result_table, aes(x = Frequency,
                               y = Absorbance,
                               color = Ear)) +
  geom_line()
```
...
```{r}
dplyr_table <- tbl(con, "Measurements") %>%
  filter(Identifier == "Rosowski_2012", SubjectNumber == 3) %>%
  collect()

ggplot(data = dplyr_table, aes(x = Frequency,
                               y = Absorbance,
                               color = Ear)) +
  geom_line()
```
...
I prefer the first approach because I can work entirely in SQL without having to think about converting formats as R will do this for me if I should specify things in the chunk.
