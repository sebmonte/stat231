...
## For radio button
size_choice_values <- c("price", "weight", "battery", "price")
size_choice_names <- c("Price", "Weight", "Battery", "Price")
...
server <- function(input, output){
  ...
  output$scatter <- renderPlot({
    skateboards %>%
      filter(drive != "Direct") %>%
      ggplot(aes_string(x = "range", y = "top_speed", color = input$pt_size)) +
