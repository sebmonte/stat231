...
unif_vec <- runif(n = 5000, min = 5, max = 25)
...
binom_vec <- rbinom(n = 1000, size = 10, p = 0.5)
...
Solution: Yes, the two variables look to have no clear relationship.
...
Solution: Yes, they look like marginal distributions.
...
Solution: The range of possible sample means goes up as the sample size goes down.
...
Solution: We expect that 5% of p values will be significant, and we do indeed find that our simulated results place the proportion at about 5%.
...
Solution: The probability of falsely rejecting the null for the first predictor is .048. The probability of falsely rejecting the null for the other 99 predictors is around .05. Out of 1000 simulations, we found that the probability of at least one being significant is .987, so this is why running a regression with 100 predictors is a problem: you will falsely find significance where there is none.
...
Solution: 1) Modify the above code to pull statistically significant variables. 2) Code to t a model that includes each of the predictors in the bivariate models 3) Extract the p value from the test for that model
...
# Grab predictors with significant p-values and paste together
sig_predictors <- paste(paste0("X", which(pvalues < 0.05)), collapse = " + ")
# Fit overall model
mod_overall <- lm(formula = paste("y ~ ", sig_predictors), data = dat)
# Extract overall p-value
broom::glance(mod_overall)$p.value
```
...
# (repeat many times and summarize results) #
#############################################
# Initialize vector for storing the overall p-values
pvalues_overall <- rep(NA, n_sim)
for(i in 1:n_sim){
# Generate 100 predictors from multivariate Normal distribution
xs <- MASS::mvrnorm(n = n_obs, mu = rep(0, n_x), Sigma = diag(n_x)) %>%
data.frame()
...
# Find indices of pvalues that are less than 0.05
sig_indices <- which(pvalues < 0.05)
# If there are no significant predictors, set overall p-value to NA
# Otherwise, fit overall model and grab overall p-value
if(length(sig_indices) == 0){
# cannot fit final regression model
pvalues_overall[i] <- NA
} else{
# Grab predictors with significant p-values and paste together
sig_predictors <- paste(paste0("X", sig_indices), collapse = " + ")
# Fit overall model
mod_overall <- lm(formula = paste("y ~ ", sig_predictors), data = dat)
# Extract overall p-value
pvalues_overall[i] <- broom::glance(mod_overall)$p.value}}
```
...
Solution: The majority of p values are less than .01, 98.7% where less than .05, and 75.8% were less than .0001. Thus, even when there is no association between predictors and the outcome using this process would result in a very statistically significant model. This is definitely not ethical, since none of the predictors are actually associated yet this process indicates that they are!

```{r}
p <- ggplot(data = data.frame(pvalues_overall), aes(x = pvalues_overall)) +
geom_density()

data.frame(pvalues_overall) %>%
filter(!is.na(pvalues_overall)) %>%
mutate(sig05 = ifelse(pvalues_overall < 0.05, 1, 0),
sig001 = ifelse(pvalues_overall < 0.001, 1, 0),
sig0001 = ifelse(pvalues_overall < 0.0001, 1, 0)) %>%
summarize(# proportion of overall tests significant at 0.05 level
prop05 = sum(sig05)/n_sim,
# proportion of overall tests significant at 0.001 level
prop001 = sum(sig001)/n_sim,
# proportion of overall tests significant at 0.0001 level
prop0001 = sum(sig0001)/n_sim)
