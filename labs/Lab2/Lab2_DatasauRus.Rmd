...
There are a total of 142 observations in each data set.
...
Solution: x and y are negatively correlated, but the correlation is so weak that we can conclude that there is not much an association between the x and y variables.
...
Solution: 
The relationship between x and y appears to be in a unique pattern, a drawing of a dinosaur, and the data points do not seem related beyond that, and so correlation is not appropriate.
...
Like the weak correlation found in the dino dataset, the star dataset
had a very weak correlation of just -.06, meaning there was barely an association between x and y.
...
Unlike the plot for the dino set, the plot for the star data set is 
in the pattern of a star. Like the dino set, correlation is not an
appropriate measure since there is no clear association between the variables
and rather they look intentionally selected to draw a pattern.
...
The correlation coefficient for this dataset is again very similar,
representing a very weak negative association between x and y.
...
```{r circ-plot, eval = TRUE, fig.width = 2.5, fig.height = 2}
ggplot(data = circle_data, mapping = aes(x = x, y = y)) +
  geom_point()
```

This third plot is in a pattern like the dino and star dataset, but this
time it is in the shape of a circle.
...
Solution: All of these data sets are intended to fit certain patterns
or shapes. All of them have similar, extremely weak correlation coefficients, which indicate that there is a very little association between the x and y variables in these datasets, making correlation an inappropriate measure here.



> part c - What do you think `ncol = 3` does in the code above? What about `digits = 3`?

Solution: I think ncol sets the number of columns that all the graphs
will be displayed in. I think digits sets the amount of digits you can have in your columns.
...
```{r test-chunk-options, echo = FALSE, collapse = TRUE, eval = TRUE, include = TRUE}
...
Solution:The $x$ value in the dino dataset has a standard deviation of `r sd(dino_data$x) %>% round(1)`.
...
```{r}
# We want the correlation between Sepal.Length and Sepal.Width 
# Get this correlation for all three iris species in the data set
data(iris)
iris %>% 
