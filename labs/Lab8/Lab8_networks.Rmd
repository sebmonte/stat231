...
Solution:  There are 226 nodes in this network, and 13805 edges.
...
mysubset <- c("GRC", "TUR", "SYR", "LBY", "ITA", "MKD", "USA",
              "EGY", "DEU", "MAR")
...
migration_sub_igraph <- graph_from_data_frame(migration_sub,
           directed = TRUE)
summary(migration_igraph)

migration_sub_network <- ggnetwork(migration_sub_igraph)
head(migration_network)

ggplot(data = migration_sub_network, aes(x = x, y = y, 
          xend = xend, yend = yend)) +
 geom_edges(arrow = arrow(type = "closed", length = unit(8, "pt")),
    color = "lightgray") +
 geom_nodes() +
...
Solution: This would be an edge attribute.
...
Solution: In this case I think the first solution that adjusts the thickness of the lines is more effective, the colors in this case are too similar and it's harder to see the overall trends.
...
Solution: In my case this solution is about the same, but I would guess that in general it would be less effective because there's many more colors and so more convoluted to follow.
...
Solution: The most central countries to the migration network were Germany, Italy, Turkey, and the USA. When you look only at outgoing edges, this widens to include Turkey and Greece, and when you look only at incoming edges it widens to include Turkey and Syria.
...
Solution: After considering weighted edges, the USA clearly stands out as having the highest degree centrality, followed by Italy and Germany. However, when considering only outgoing edges, Germany and Italy are highest, while when considering oncoming edges, the USA and Turkey are the highest.
...
Solution: First, we would need to find the location of all the occurrences of the names we are interested in examining. Next, we would need to 



> part b - How many GoT characters (nodes) and character interactions (edges) are in this network? 

Solution: There are 107 nodes and 352 edges.
...
# Get descriptions and counts of vertices
V(got_igraph) # not necessarily useful to print
vcount(got_igraph)

# Get descriptions and counts of edges
E(got_igraph) # not necessarily useful to print
ecount(got_igraph)
...
Solution: The density of this graph is .062, indicating that it is not very dense at all compared to the proportion of possible edges that there could be.
...
Solution: This graph is connected, meaning there is path between every single pair of vertices.
...
Solution: The diameter is 6, which indicates that the longest geodesic between any two characters is 6 characters away.
...
Solution: Tyrion, Jon, Sansa, Jaime, and Robb are the characters with the highest degree. Tyrion, Jon, Sansa, Jaime, and Bran are the characters with the highest weighted degree. These values do match figure 3.
...
Solution: Degree counts how many characters are interacted with, not how many interactions occur. Robb has more interactions with Bran, but Bran interacts with the individuals he does interact with many more times.
...
Solution: Jon, Robert, Tyrion, Daenerys, and Robb are the top ranked characters on betweenness centrality.
...
Solution: Tyrion, Rob, Sansa, and Daenerys are in the top 5 using the page rank measure.
...
Daenerys interacts only with a certain small set of characters, and is far away from other characters in the novel. However, since she is the only connection to many of these characters, she acts to connect them together to a wider network of characters.
...
got_igraph <- set_vertex_attr(got_igraph, name = "pagerank", value = got_stats$pagerank)
# Make into ggnetwork object
got_network <- ggnetwork(got_igraph)
set.seed(231)
# Graph network
ggplot(data = got_network,
aes(x = x, y = y, xend = xend, yend = yend)) +
geom_edges(color = "lightgray", curvature = 0.2) +
# for whatever reason, this will not compile without the $ syntax
# which it actively discourages
geom_nodes(aes(size = got_network$pagerank)) +
geom_nodelabel_repel(aes(label = name)) +
labs(size = "Page Rank") +
theme_blank()
...
Solution: The modularity is .4896.
...
Solution: There are 6 communities,  The 1st has 25 characters, 2nd has 37, third has only 8, fourth has 15, 5th has 
8, and the 6th has 14.
...
got_igraph <- set_vertex_attr(got_igraph,
name = "membership",
value = got_membership)
got_network <- ggnetwork(got_igraph) %>%
mutate(membership = factor(membership))
# Create a plot
set.seed(231) # may not be needed
got_network %>%
ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
geom_edges(color = "gray50", curvature = 0.2) +
# for whatever reason, this will not compile without the $ syntax
# which it actively discourages
geom_nodes(aes(size = got_network$pagerank, color = membership)) +
geom_nodelabel_repel(aes(label = name, color = membership)) +
labs(size = "Page Rank") +
guides(color = "none") +
theme_blank()
