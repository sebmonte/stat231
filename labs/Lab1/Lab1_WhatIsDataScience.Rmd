...
date: "For class Tuesday, February 8th"
output:
  pdf_document:
    fig_height: 3
    fig_width: 5
...
Everyone in the group should pick an application area of interest to them.  Have everyone do a google search for "data science application area" where you put your application area in. For example, I like learning about archaeology, so I searched for "data science archaeology".
...
## What do you think one case/observation in our final unvotes data set represents? A country? A UN-session?

\newpage

```{r view-un_votes_final}
head(unvotes)
```
...
```{r plot-yearly-yes-issue, warning=FALSE, message=FALSE, fig.height = 5, fig.width = 7}
unvotes %>%
