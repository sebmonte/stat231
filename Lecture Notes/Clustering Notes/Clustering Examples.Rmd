...
```{r}
set.seed(231)
#Assign clustering variables into vector to pass them into select
clustering_vars <- c("CL", "RW")
crabs_km2 <- crabs %>% 
  select(clustering_vars) %>% 
#Takes a dataset as input, passing two wanted variables in
...
crabs <- crabs %>%
#Factor needed to treat this as categorical
...
#For any variable that is numeric, apply the scale function, and take the name of the original variable and add 'scaled'. subtracts mean and divides by SD
crabs <- crabs %>% 
...
```{r}
#Tells us the tightness between a cluster and points in the cluster. More clusters = easier to put points closer to center, decreasing this value. 
